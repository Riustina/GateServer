...
void MySqlDao::EnsureBaseTables()
{
    auto con = pool_->getConnection();
    if (con == nullptr) {
        std::cerr << "[MySqlDao.cpp] 函数 [EnsureBaseTables()] 无法获取数据库连接" << std::endl;
        return;
    }
    ...
    try {
        ...
        std::unique_ptr<sql::ResultSet> res(stmt->executeQuery("SELECT COUNT(*) AS cnt FROM `user_id`"));
        if (res->next() && res->getInt("cnt") == 0) {
            stmt->execute("INSERT INTO `user_id` (`id`) VALUES (0)");
            std::cout << "[MySqlDao.cpp] 函数 [EnsureBaseTables()] 已初始化 user_id 种子数据" << std::endl;
        }
    ...
    {
        std::cerr << "[MySqlDao.cpp] 函数 [EnsureBaseTables()] SQLException: " << e.what();
        ...
        try {
            ...
            std::cerr << "[MySqlDao.cpp] 函数 [EnsureBaseTables()] 返回连接到池中时发生异常" << std::endl;
        }
    }
    catch (...)
    {
        std::cerr << "[MySqlDao.cpp] 函数 [EnsureBaseTables()] 未知异常" << std::endl;
        try {
            if (con) {
                pool_->returnConnection(std::move(con));
            }
...
void MySqlDao::EnsureRegUserProcedure()
{
    auto con = pool_->getConnection();
    if (con == nullptr) {
        std::cerr << "[MySqlDao.cpp] 函数 [EnsureRegUserProcedure()] 无法获取数据库连接" << std::endl;
        return;
    }
    ...
    try {
        ...
        stmt->execute(
            ...
            "        INSERT INTO `user` (`uid`, `name`, `email`, `pwd`) VALUES (new_uid, new_name, new_email, new_pwd); "
            "        SET result = new_uid; "
            "        COMMIT; "
            "    END IF; "
            "END");

        std::cout << "[MySqlDao.cpp] 函数 [EnsureRegUserProcedure()] 已确保 reg_user 存储过程存在" << std::endl;
        pool_->returnConnection(std::move(con));
    }
    catch (sql::SQLException& e)
    {
        std::cerr << "[MySqlDao.cpp] 函数 [EnsureRegUserProcedure()] SQLException: " << e.what();
        std::cerr << " (MySQL error code: " << e.getErrorCode();
        std::cerr << ", SQLState: " << e.getSQLState() << " )" << std::endl;
        try {
            if (con) {
            ...
            std::cerr << "[MySqlDao.cpp] 函数 [EnsureRegUserProcedure()] 返回连接到池中时发生异常" << std::endl;
        }
    }
    catch (...)
    {
        std::cerr << "[MySqlDao.cpp] 函数 [EnsureRegUserProcedure()] 未知异常" << std::endl;
        try {
            if (con) {
                pool_->returnConnection(std::move(con));
            }
