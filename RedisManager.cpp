...
struct ConnectionGuard {
    ...
    ConnectionGuard(RedisConPool* p, redisContext* c)
        : pool(p), ctx(c) {}
...
bool RedisManager::Set(const std::string& key, const std::string& value)
{
    ...
    if (!g.ctx) {
        std::cerr << "[RedisManager.cpp] Set [SET] 获取连接失败\n";
        return false;
    }
    ...
    if (!reply) {
        std::cerr << "[RedisManager.cpp] Set [SET] 未收到响应，Key: " << key << "\n";
        g.ctx->err = 1;
        return false;
    }
...
bool RedisManager::Get(const std::string& key, std::string& value)
{
    ...
    ConnectionGuard g(pool_.get(), pool_->getConnection());
    if (!g.ctx) {
        std::cerr << "[RedisManager.cpp] Get [GET] 获取连接失败\n";
        return false;
    }

    redisReply* reply = (redisReply*)redisCommand(
        g.ctx, "GET %b", key.c_str(), (size_t)key.length());
    if (!reply) {
        std::cerr << "[RedisManager.cpp] Get [GET] 未收到响应，Key: " << key << "\n";
        g.ctx->err = 1;
        return false;
    }
...
bool RedisManager::LPush(const std::string& key, const std::string& value)
{
    ...
    ConnectionGuard g(pool_.get(), pool_->getConnection());
    if (!g.ctx) {
        std::cerr << "[RedisManager.cpp] LPush [LPUSH] 获取连接失败\n";
        return false;
    }
    ...
    redisReply* reply = (redisReply*)redisCommand(
        ...
        value.c_str(), (size_t)value.length());
    if (!reply) {
        std::cerr << "[RedisManager.cpp] LPush [LPUSH] 未收到响应，Key: " << key << "\n";
        g.ctx->err = 1;
        return false;
    }
...
bool RedisManager::LPop(const std::string& key, std::string& value)
{
    ...
    ConnectionGuard g(pool_.get(), pool_->getConnection());
    if (!g.ctx) {
        std::cerr << "[RedisManager.cpp] LPop [LPOP] 获取连接失败\n";
        return false;
    }

    redisReply* reply = (redisReply*)redisCommand(
        g.ctx, "LPOP %b", key.c_str(), (size_t)key.length());
    if (!reply) {
        std::cerr << "[RedisManager.cpp] LPop [LPOP] 未收到响应，Key: " << key << "\n";
        g.ctx->err = 1;
        return false;
    }
    ...
    case REDIS_REPLY_STRING:
        ...
        break;
    case REDIS_REPLY_NIL:
        std::cout << "[RedisManager.cpp] LPop [LPOP] 列表为空，Key: " << key << "\n";
        break;
    case REDIS_REPLY_ERROR:
...
bool RedisManager::RPush(const std::string& key, const std::string& value)
{
    ...
    ConnectionGuard g(pool_.get(), pool_->getConnection());
    if (!g.ctx) {
        std::cerr << "[RedisManager.cpp] RPush [RPUSH] 获取连接失败\n";
        return false;
    }
    ...
    redisReply* reply = (redisReply*)redisCommand(
        ...
        value.c_str(), (size_t)value.length());
    if (!reply) {
        std::cerr << "[RedisManager.cpp] RPush [RPUSH] 未收到响应，Key: " << key << "\n";
        g.ctx->err = 1;
        return false;
    }
...
bool RedisManager::RPop(const std::string& key, std::string& value)
{
    ...
    ConnectionGuard g(pool_.get(), pool_->getConnection());
    if (!g.ctx) {
        std::cerr << "[RedisManager.cpp] RPop [RPOP] 获取连接失败\n";
        return false;
    }

    redisReply* reply = (redisReply*)redisCommand(
        g.ctx, "RPOP %b", key.c_str(), (size_t)key.length());
    if (!reply) {
        std::cerr << "[RedisManager.cpp] RPop [RPOP] 未收到响应，Key: " << key << "\n";
        g.ctx->err = 1;
        return false;
    }
    ...
    case REDIS_REPLY_STRING:
        ...
        break;
    case REDIS_REPLY_NIL:
        std::cout << "[RedisManager.cpp] RPop [RPOP] 列表为空，Key: " << key << "\n";
        break;
    case REDIS_REPLY_ERROR:
...
bool RedisManager::HSet(std::string_view key, std::string_view field, std::string_view value)
{
    ...
    ConnectionGuard g(pool_.get(), pool_->getConnection());
    if (!g.ctx) {
        std::cerr << "[RedisManager.cpp] HSet [HSET] 获取连接失败\n";
        return false;
    }
    ...
    if (!reply) {
        std::cerr << "[RedisManager.cpp] HSet [HSET] 未收到响应，Key: " << key << "\n";
        g.ctx->err = 1;
        return false;
    }
...
bool RedisManager::HGet(std::string_view key, std::string_view field, std::string& value)
{
    ...
    ConnectionGuard g(pool_.get(), pool_->getConnection());
    if (!g.ctx) {
        std::cerr << "[RedisManager.cpp] HGet [HGET] 获取连接失败\n";
        return false;
    }
    ...
    redisReply* reply = (redisReply*)redisCommand(
        ...
        field.data(), field.size());
    if (!reply) {
        std::cerr << "[RedisManager.cpp] HGet [HGET] 未收到响应，Key: " << key << "\n";
        g.ctx->err = 1;
        return false;
    }
...
bool RedisManager::Del(std::string_view key)
{
    ...
    ConnectionGuard g(pool_.get(), pool_->getConnection());
    if (!g.ctx) {
        std::cerr << "[RedisManager.cpp] Del [DEL] 获取连接失败\n";
        return false;
    }

    redisReply* reply = (redisReply*)redisCommand(g.ctx, "DEL %b", key.data(), key.size());
    if (!reply) {
        std::cerr << "[RedisManager.cpp] Del [DEL] 未收到响应，Key: " << key << "\n";
        g.ctx->err = 1;
        return false;
    }
...
bool RedisManager::ExistsKey(std::string_view key)
{
    ...
    ConnectionGuard g(pool_.get(), pool_->getConnection());
    if (!g.ctx) {
        std::cerr << "[RedisManager.cpp] ExistsKey [EXISTS] 获取连接失败\n";
        return false;
    }

    redisReply* reply = (redisReply*)redisCommand(g.ctx, "EXISTS %b", key.data(), key.size());
    if (!reply) {
        std::cerr << "[RedisManager.cpp] ExistsKey [EXISTS] 未收到响应，Key: " << key << "\n";
        g.ctx->err = 1;
        return false;
    }
